...
# initialize models
policy_model = ConvModel(side=env._side, n_actions=config.NUM_ACTIONS, zero_padding=False, deeper=True, batch_norm=True)
target_model = ConvModel(side=env._side, n_actions=config.NUM_ACTIONS, zero_padding=False, deeper=True, batch_norm=True)

policy_model.build(input_shape=(None, env._side, env._side, 3))
target_model.build(input_shape=(None, env._side, env._side, 3))
