...
# make the env
env = PongWrapper(points_per_episode=config.POINTS_PER_EPISODE)

# set constants
config.NUM_ACTIONS = env._ACTION_SPACE_SIZE
config.ALL_ACTIONS = np.array(list(range(config.NUM_ACTIONS)))
config.MAX_TIME_STEPS = env._step_limit_per_episode

# initialize models
policy_model = create_model(side=env._LOWEST_SIDE, n_actions=config.NUM_ACTIONS, weights_dir=config.CONTINUE_MODEL)
# policy_model = PongModel(side=env._LOWEST_SIDE, n_actions=config.NUM_ACTIONS)
target_model = PongModel(side=env._LOWEST_SIDE, n_actions=config.NUM_ACTIONS)
...
for episode in tqdm(range(config.EPISODES), ascii=True, unit="episodes"):
    # get initial state
    current_state = tf.convert_to_tensor(scale_states(env.reset()[0]), dtype=tf.float32)
    ...
    while not (terminated or truncated):
        ...
        if reward > env._DEFAULT_REWARD:
            print(f"Won episode {episode} game {env._episode_tot_abs_reward}")
            somebody_won = True
        elif reward < env._DEFAULT_REWARD:
            somebody_won = True
        else:
            if truncated:
        ...
        next_state = tf.convert_to_tensor(scale_states(obs), dtype=tf.float32)

        replay_buffer.push(current_state=current_state,
                           action=action,
                           next_state=next_state,
                           reward=tf.convert_to_tensor([float(reward)], dtype=tf.float32),
                           terminated=tf.convert_to_tensor([somebody_won])
                           )
